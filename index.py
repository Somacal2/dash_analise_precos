...
df_main.drop(['UNIDADE DE MEDIDA', 'COEF DE VARIAÇÃO REVENDA', 'COEF DE VARIAÇÃO DISTRIBUIÇÃO', 'NÚMERO DE POSTOS PESQUISADOS',
                        'DATA INICIAL', 'DATA FINAL', 'PREÇO MÍNIMO DISTRIBUIÇÃO', 'PREÇO MÁXIMO DISTRIBUIÇÃO',
                        'PRODUTO', 'PREÇO MÉDIO DISTRIBUIÇÃO', 'MARGEM MÉDIA REVENDA', 'PREÇO MÍNIMO REVENDA',
                        'PREÇO MÁXIMO REVENDA', 'DESVIO PADRÃO REVENDA', 'DESVIO PADRÃO DISTRIBUIÇÃO'], inplace=True, axis=1)
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    dbc.Row([
                        dbc.Col([
                            html.Legend("Relatório de Vendas", style={'font-size': '110%', 'font-weight': 'bold'})
                        ], sm=8),
                        dbc.Col([
                            html.I(className="fa fa-chart-bar", style={'font-size': '200%'})
                        ], sm=4, align="center")
                    ]),
                    dbc.Row([
                        dbc.Col([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    html.H6('Qual preço é menor em um dado príodo de tempo?'),
                    dbc.Row([
                        dbc.Col([
                            dcc.Dropdown(
                            id='select_estado1',
                        ...
                        ], sm=10, md=6, style={'font-size': '80%'}),
                    ], style={'margin-top': '20px'}, justify='center'),
                    dcc.Graph(id='direct-comparison-graph', config={"displayModeBar": False, "showTips": False}),
        ...
        dbc.Col([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            dcc.Graph(id='card2-indicators', config={"displayModeBar": False, "showTips": False}, style={'margin-top': '30px'})
                        ])
                    ], style=tab_card)
                ])
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                ...
                ], className='g1', style={'height': '20%', 'justify-content': 'center'})
            ], style=tab_card)
        ])
    ], className='main_row g-2 my-auto')
...
def graph1(data, ano, regiao, toggle):
    ...
    fig2_text = [f'{y} - R${x}' for x,y in zip(dff_estado.ESTADO.unique(), dff_estado['VALOR REVENDA (R$/L)'].unique())]
...
def func(data, est1, est2, toggle):
    ...
    fig.add_scatter(name=est1, x=df_final['DATA'], y=df_final['VALOR REVENDA (R$/L)'])

    fig.add_scatter(name=est2, x=df_final['DATA'], y=df_final['VALOR REVENDA (R$/L)'].where(df_final['VALOR REVENDA (R$/L)'] > 0.00000))

    fig.update_layout(main_config, height=350, template=template)
    fig.update_yaxes(range = [-0.7,0.7])

    #Annotation para mostar quem é o mais barato
...
def card1(data, estado, toggle):
    ...
    fig.add_trace(go.Indicator(
        mode = "number+delta",
        title= {"text": f"<span style='size:60%'>{estado}</span><br><span style='sont-size:0.7em'>{data1} - {data2}</span>"},
...
@app.callback(
    Output("card2-indicators", "figure"),
    [Input('dataset', 'data'),
    Input('select_estado2', 'value'),
    Input(ThemeSwitchAIO.ids.switch("theme"), "value")]
)
